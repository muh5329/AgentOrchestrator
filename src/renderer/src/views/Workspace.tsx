...
export function WorkspaceView(): React.JSX.Element {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <Console projectId={projectId} agentId={agentFilter || null} />
    </div>
  )
}
...
function Console({
  ...
  useEffect(() => {
    return window.ao.onEvent((raw) => {
      ...
      if (event.type === 'CONSOLE_OUTPUT') {
        setLines((current) =>
          [...current, { text: String(event.message ?? ''), stream: String(event.data?.stream ?? 'stdout') }].slice(-500)
        )
      }
      if (event.type === 'CONSOLE_EXIT') {
        setLines((current) => [
          ...current,
          { text: `\n[exit ${String(event.data?.exitCode)}]\n`, stream: 'meta' }
        ])
  ...
  const run = async (): Promise<void> => {
    ...
    setLines((current) => [...current, { text: `$ ${command}\n`, stream: 'meta' }])
    setHistory((h) => [command, ...h].slice(0, 50))
    const started = await api.console.run(projectId, command, agentId)
  ...
  return (
    ...
    >
      <header className="flex h-8 shrink-0 items-center gap-2 px-2">
        <button
          className="text-2xs uppercase tracking-wider text-ink-faint hover:text-ink-dim"
          onClick={() => setOpen(!open)}
        >
          Console
        </button>
        {session?.running && <StatusDot status="RUNNING" />}
        <div className="flex-1" />
        {session?.running && (
          <Button size="sm" variant="ghost" onClick={() => void api.console.kill(session.id)}>
            Stop
          </Button>
        )}
        {open && (
      ...
      {open && (
        <>
          <div ref={outputRef} className="scroll-y min-h-0 flex-1 bg-base-900 px-3 py-1.5">
            {lines.length === 0 && (
              <p className="mono text-2xs text-ink-faint">
                Runs commands in this workspace. Not a full terminal - no TTY, so interactive
                programs will not behave as they do in your shell.
              </p>
            )}
            {lines.map((line, i) => (
              <pre
                key={i}
                className={clsx(
                  'mono whitespace-pre-wrap text-2xs leading-relaxed',
                  line.stream === 'stderr' && 'text-warn',
                  line.stream === 'meta' && 'text-accent'
                )}
              >
                {line.text}
              </pre>
          ...
          <div className="flex items-center gap-2 border-t border-edge px-2 py-1.5">
            ...
            <input
              ...
              className="mono h-6 flex-1 border-0 bg-transparent text-xs"
            />
