          ...
          {Object.keys(properties).length === 0 ? (
            ...
            Object.entries(properties).map(([key, spec]) => (
              <label key={key} className="mb-3 block">
                ...
                {spec.enum?.length ? (
                  // The schema already says what the valid answers are, so the
                  // form offers those rather than letting a typo reach the tool.
                  <select
                    data-arg={key}
                    value={values[key] ?? ''}
                  ...
                  >
                    <option value="">{required.includes(key) ? 'Choose one…' : '(default)'}</option>
