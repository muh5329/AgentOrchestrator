...
npm test             # 146 tests: unit, integration, workflow, git, providers and restart-recovery
...
`ProviderAdapter` is the seam. Four implementations ship:

| Adapter | What it is |
| --- | --- |
| `claude-code` | Spawns the local Claude Code CLI, on your Claude subscription. Orchestration tools reach it as an MCP server over a loopback control plane, so tool calls still flow through the permission gate. Agent permissions map onto the CLI's own tools — an agent without `FILES_WRITE` genuinely cannot write. |
| `local` | Any OpenAI-compatible server on this machine: LM Studio, Ollama, llama.cpp, vLLM. Drives its own tool loop, so calls still go through the gate. Costs nothing and says so. |
...
The Claude Code CLI's documented precedence puts an API key **above** the subscription you signed
in with, and in headless mode it uses the key whenever one is present. So an `ANTHROPIC_API_KEY`
exported in the shell that happened to launch this app would quietly bill pay-as-you-go credits to
somebody already paying for a plan — and the first they would hear of it is "credit balance is too
low" halfway through a run.

That is a decision about someone's money, so it is made deliberately. Settings → Providers carries
the choice, the subscription is the default, and the redirecting variables (`ANTHROPIC_API_KEY`,
`ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_BASE_URL`, the Bedrock/Vertex/Foundry switches) are kept out of
the spawned environment unless you ask for API billing. The panel states which account a run would
use *before* you spend anything, and says when a key is present and being held back.

![The billing account, with a key present in the environment and deliberately held back](screenshots/billing.png)

### Local models

One adapter, not one per vendor: LM Studio, Ollama, llama.cpp's server and vLLM all speak the same
OpenAI-shaped `/v1/chat/completions` with the same tool-calling envelope, so what varies between them
is a base URL — and a base URL is configuration, not code. Settings → Local models will find a server
already listening on the usual ports, and the model list comes from the server itself, so what you
can pick is what is actually loaded.

Tool calls from a local model go through the same `ToolRuntime` gate as everything else: a local
model is not a way around permissions or approvals. Runs are recorded at zero cost, because they
are, but tokens are still counted — that is what tells you a model is looping. A model without
tool-calling support cannot do agent work, and the provider says so plainly rather than reporting a
clean run in which nothing happened.
...
tests/providers.test.ts    which account a run is billed to, and what is kept out of its environment
tests/local-model.test.ts  the local provider against a real OpenAI-compatible server
tests/floor.test.ts        the office keeps its state; robots walk rather than teleport
tests/markdown.test.ts     the document parser always terminates
```
