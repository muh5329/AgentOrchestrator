...
import type { AgentBlueprint, AgentTemplate } from '@shared/agent-templates'
...
export const api = {
  ...
  blueprints: {
    templates: () => invoke<AgentTemplate[]>('blueprints.templates'),
    export: (agentId: string) => invoke<AgentBlueprint>('blueprints.export', { agentId }),
    import: (projectId: string, json: string, name?: string) =>
      invoke<Agent>('blueprints.import', { projectId, json, name }),
    fromTemplate: (projectId: string, templateId: string, name?: string) =>
      invoke<Agent>('blueprints.fromTemplate', { projectId, templateId, name })
  },
  agents: {
  ...
  executions: {
    ...
    byAgent: (agentId: string, limit = 40) =>
      invoke<Array<Omit<Execution, 'transcript'> & { transcriptLength: number }>>(
        'executions.byAgent',
        { agentId, limit }
      ),
  ...
  providers: {
    ...
    billing: () => invoke<BillingState>('providers.billing'),
  ...
  local: {
    config: () => invoke<LocalConfig>('local.config'),
    save: (input: Record<string, string>) => invoke<LocalConfig>('local.save', input),
    models: (baseUrl?: string) => invoke<string[]>('local.models', { baseUrl }),
    detect: () => invoke<Array<{ label: string; baseUrl: string; models: string[] }>>('local.detect')
  },

  mail: {
    config: () => invoke<MailConfig>('mail.config'),
    save: (input: Record<string, string>) => invoke<MailConfig>('mail.save', input),
    verify: () => invoke<{ ok: boolean; detail: string }>('mail.verify')
  },
  dialog: {
    pickFolder: (defaultPath?: string) =>
      invoke<{ path: string | null }>('dialog.pickFolder', { defaultPath })
  },
  system: {
    info: () =>
      invoke<{
