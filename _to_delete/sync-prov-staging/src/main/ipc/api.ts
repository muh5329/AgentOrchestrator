...
import { detectLocalServers, listModels } from '../runtime/providers/local-openai'
...
export function createApi(ctx: AppContext): Record<string, (payload: Payload) => unknown> {
  return {
    ...
    /* ---------------------------- blueprints --------------------------- */
    'blueprints.templates': () => ctx.blueprints.templates(),
    'blueprints.export': (p) => ctx.blueprints.export(p.agentId),
    'blueprints.import': (p) =>
      ctx.blueprints.import({
        projectId: p.projectId,
        json: p.json,
        parentAgentId: p.parentAgentId ?? null,
        nameOverride: p.name
      }),
    'blueprints.fromTemplate': (p) =>
      ctx.blueprints.fromTemplate({
        projectId: p.projectId,
    ...
    /* ------------------------------- mail ------------------------------ */
    'mail.config': () => ctx.mail.config(),
    'mail.save': (p) => ctx.mail.save(p as never),
    'mail.verify': () => ctx.mail.verify(),
    ...
    /**
     * An agent's own run history, newest first.
     *
     * The transcript can be large, so it is dropped here and fetched per run by
     * `executions.get` - the list is for finding the run that went wrong, not
     * for reading all of them at once.
     */
    'executions.byAgent': (p) =>
      ctx.db
        .select()
        .from(executionsTable)
        .where(eq(executionsTable.agentId, p.agentId))
        .orderBy(desc(executionsTable.startedAt))
        .limit(Math.min(Number(p.limit ?? 40), 200))
        .all()
        .map(({ transcript, ...rest }) => ({
          ...rest,
    ...
    /**
     * Who a Claude Code run started right now would be billed to, and what the
     * person asked for. The two differ when the environment overrides a choice,
     * which is exactly the case worth showing.
     */
    'providers.billing': () => {
      const mode =
        ctx.providers.getSecret('claude.billingMode') === 'api-key' ? 'api-key' : 'subscription'
      const adapter = ctx.providers.has('claude-code')
        ? (ctx.providers.get('claude-code') as {
            billing?: () => { account: string; detail: string; cause?: string }
          })
        : null
      const verdict = adapter?.billing?.() ?? {
        account: 'subscription',
        detail: 'The Claude Code CLI provider is not registered.'
      }
      return { ...verdict, mode }
    },
    ...
    /* -------------------------- local models --------------------------- */
    'local.config': () => ({
      baseUrl: ctx.providers.getSecret('local.baseUrl') ?? '',
      model: ctx.providers.getSecret('local.model') ?? '',
      hasKey: Boolean(ctx.providers.getSecret('local.apiKey'))
    }),
    'local.save': (p) => {
      for (const key of ['baseUrl', 'model', 'apiKey'] as const) {
        const value = p[key]
        if (value !== undefined) {
          ctx.providers.setSecret(`local.${key}`, String(value) || null)
        }
      }
      return {
        baseUrl: ctx.providers.getSecret('local.baseUrl') ?? '',
    ...
    /** Everything the configured server is serving, straight from the server. */
    'local.models': async (p) =>
      listModels(
        String(p.baseUrl ?? ctx.providers.getSecret('local.baseUrl') ?? ''),
        ctx.providers.getSecret('local.apiKey')
      ),
    /** Finds a server that is already running, so nobody has to type a URL. */
    'local.detect': () => detectLocalServers(),
