...
import { LocalModelAdapter } from '../runtime/providers/local-openai'
...
export function bootstrap(options: BootstrapOptions): BootstrappedApp {
  ...
  ctx.blueprints = new BlueprintService(ctx)
  ctx.mail = new MailService(ctx)
  ...
  ctx.providers.register(
    new ClaudeCodeAdapter({
      ...
      // Subscription unless someone deliberately says otherwise. The CLI
      // prefers any API key it can see, which would spend credits belonging to
      // a person already paying for a plan.
      billingMode: () =>
        ctx.providers.getSecret('claude.billingMode') === 'api-key' ? 'api-key' : 'subscription',
      apiKey: () => ctx.providers.getSecret('anthropic.apiKey')
    })
  )
  ctx.providers.register(
  ...
  ctx.providers.register(
    new LocalModelAdapter(() => ({
      ...
      apiKey: ctx.providers.getSecret('local.apiKey'),
      defaultModel: ctx.providers.getSecret('local.model') ?? ''
    }))
  )
