...
/* ------------------------------------------------------------------ */
/* One world per project, outliving the component that draws it        */
/* ------------------------------------------------------------------ */

/** The bits of the floor that belong to the session rather than to a mount. */
export interface FloorWorld {
  sim: FloorSim
  /** Event and message ids already spoken, so a remount is not a shouting match. */
  spoken: Set<string>
  /** View controls, so switching tabs does not undo what you set. */
  view: { playing: boolean; speed: number; zoom: number; hud: boolean }
}

const worlds = new Map<string, FloorWorld>()

/** How many projects' floors to keep. Small: each is a few dozen actors. */
const KEEP = 8

/**
 * The floor for a project, created once and kept.
 *
 * The office is a place, and a place does not rebuild itself because you looked
 * away. Holding this in the component meant every tab switch unmounted the
 * world: robots walked in from the door again as though they had just been
 * hired, and the whole event buffer replayed as a fresh burst of chatter. The
 * world belongs to the session, so it lives here and the view merely draws it.
 */
export function worldFor(projectId: string): FloorWorld {
  const existing = worlds.get(projectId)
  if (existing) {
    // Re-insert so the map orders least- to most-recently used.
    worlds.delete(projectId)
    worlds.set(projectId, existing)
    return existing
  }
  ...
  const world: FloorWorld = {
    ...
    view: { playing: true, speed: 1, zoom: 1, hud: true }
  }
  worlds.set(projectId, world)

  while (worlds.size > KEEP) {
    const oldest = worlds.keys().next().value
