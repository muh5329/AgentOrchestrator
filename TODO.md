...
### 1. Run the Claude Code CLI provider for real

**Nothing has ever executed through it.** Every test and screenshot in this repository used the
deterministic `scripted` provider, because the development container had no `claude` binary. The
adapter typechecks and the architecture is right; the wire format is unverified.

Likely to need fixing on first contact:

- `stream-json` event field names in `ClaudeCodeAdapter.consume()` — `src/main/runtime/providers/claude-code.ts`
- Whether `--mcp-config` accepts a file path in the installed CLI version, and whether
  `--strict-mcp-config` suppresses the user's own servers as intended
- The bridge handshake: `src/main/runtime/mcp/bridge-entry.ts` talking to `ControlServer`
- Cost and token extraction from the final `result` event

**Done when:** a project with one agent completes a real task, the transcript shows tool calls that
went through `ToolRuntime`, and `task_executions` has non-zero `cost_usd_micros`.
...
**Done when:** the key round-trips through the UI, and `strings agent-orchestrator.db | grep sk-ant`
finds nothing.
